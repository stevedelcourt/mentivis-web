...
| `/mentivisos/` | MentivisOS product page (AI-native learning platform) |
| `/mentivissolutions/` | Redirect 301 → `/` (removed from nav and sitemap) |
| `/homepage-old/` | Backup of old homepage |
| `/skillpath/` | Redirect 301 → `/mentivisos/` (rebranded) |
...
| `/fr/skillpath` | `/fr/mentivisos/` | 301 |
| `/en/skillpath` | `/en/mentivisos/` | 301 |
| `/fr/insights/mentivis-devoile-skillpath-os-formation-native-ia` | `/fr/insights/mentivis-devoile-mentivos-os-formation-native-ia/` | 301 |
| `/en/insights/mentivis-devoile-skillpath-os-formation-native-ia` | `/en/insights/mentivis-devoile-mentivos-os-formation-native-ia/` | 301 |
...
**Includes:**
- 18 static pages × 2 languages (home, about, enterprise, of, solutions, mentivisos, insights, guides, score-formation, careers, meeting, contact, videos, legal, privacy, terms, cgv)
- 20 insight articles × 2 languages
- **Total: ~76 URLs**
...
|------|----------|----------|
| `/` | Mentivis — Tous les métiers de la formation. De bout en bout. | Mentivis - Every training discipline. End to end. |
...
| `/mentivisos` | MentivisOS — Plateforme native IA | MentivisOS — AI-Native Learning Platform |
...
## 25. Rebranding: Skillpath → MentivisOS (2026-05-07)

### 25.1 Route Change

- **Old:** `/fr/skillpath` → **New:** `/fr/mentivisos`
- **Old:** `/en/skillpath` → **New:** `/en/mentivisos`
- Folder renamed: `src/app/[lang]/skillpath/` → `src/app/[lang]/mentivisos/`
- All nav/footer links updated to `/mentivisos`
- `.htaccess` redirects: `/skillpath` → `/mentivisos/` (301)

### 25.2 Insight Article Slug Change

- **Old:** `mentivis-devoile-skillpath-os-formation-native-ia`
- **New:** `mentivis-devoile-mentivos-os-formation-native-ia`
- Content files renamed: `skillpath-os-formation-native-ia.*` → `mentivos-os-formation-native-ia.*`
- `.htaccess` redirects: old slug → new slug (301)

### 25.3 Image Updates

- New insight image: `public/images/insights/mentivos.avif` (copied from updated hero image)
- Old image `public/images/insights/skillpath.avif` replaced with updated version
- All JSON references updated: `heroImage: "/images/insights/mentivos.avif"`

### 25.4 OG Image for MentivisOS Pages

- New file: `public/opengraph-image-mentivisOS.jpg`
- `mentivisos/page.tsx` overrides `openGraph` and `twitter` images to use this file
- Other pages continue using default `opengraph-image.jpg`

### 25.5 Favicon Update

- Replaced `public/favicon.ico` with `round-icon-512x512.png` (circular blue icon)
- Replaced `public/apple-touch-icon.png` with same file
- Google will cache the new icon for weeks before showing in search results

### 25.6 Tagline Change (Metadata Only)

- **Old:** "Mentivis - Opérateur en formation et développement des compétences"
- **New:** "Mentivis - Tous les métiers de la formation. De bout en bout."
- Changed in `src/app/[lang]/layout.tsx` `generateMetadata` only
- EN: "Mentivis - Every training discipline. End to end."

### 25.7 About Page CTA

- Changed `accent="ink"` to `accent="purple"` (deep blue `#000776`)
- Added 60px white spacer between CTA and footer

### 25.8 Files Changed (39 total)

| Category | Files |
|----------|-------|
| Routes | `skillpath/` → `mentivisos/` (page.tsx, SkillpathClient.tsx) |
| Content | `skillpath-os-formation-native-ia.*` → `mentivos-os-formation-native-ia.*` (3 files) |
| Config | `.htaccess`, `sitemap.ts`, `contact-url.ts`, `featured-insights.json` |
| Data | `insights-meta.json`, `insights.ts` |
| Components | `TopNav.tsx`, `Footer.tsx`, `AboutClient.tsx`, `CookieConsentDeferred.tsx` |
| Layout | `layout.tsx` (tagline), `HomeClient.tsx`, `ContactClient.tsx` |
| Messages | `fr.json`, `en.json`, `site-fr.txt`, `site-en.txt` |
| Assets | `favicon.ico`, `apple-touch-icon.png`, `opengraph-image-mentivisOS.jpg`, `round-icon-512x512.png`, `mentivos.avif` (2), `skillpath.avif` (updated) |
| Scripts | `build-ftp.js`, `gen-llms-txt.js` |
