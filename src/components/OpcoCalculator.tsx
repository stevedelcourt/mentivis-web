...
export default function OpcoCalculator() {
  ...
  const handleContact = useCallback(async (e: React.FormEvent) => {
    ...
    const ok = await hsSubmit(
      ...
      {
        pageUri: typeof window !== "undefined" ? window.location.href : "https://www.mentivis.com/fr/opco",
        pageName: "OPCO Calculator",
      }
    );
                ...
                {!contactSent ? (
                  <form className="sc-cta-form" onSubmit={handleContact}>
                    ...
                    <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", fontSize: 13, color: "var(--m-ink-3)", lineHeight: 1.5 }}>
                      ...
                      <span>
                        {s.ctaConsent}{" "}
                        <Link href={`/${lang}/privacy`} style={{ color: "var(--m-purple)", textDecoration: "underline" }}>{s.ctaConsentLink}</Link>.
                      </span>
                    </label>
                    {hsError && (
