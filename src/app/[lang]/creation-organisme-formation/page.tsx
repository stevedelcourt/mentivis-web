...
export default async function Page({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isFr = lang === "fr";
  const og = getOgImage("/creation-organisme-formation", lang);
  // Single source of truth for FAQ: visible HTML and JSON-LD render from here (verbatim match)
  const faq: Array<{ q: { fr: string; en: string }; a: { fr: string; en: string } }> = [
    {
      q: { fr: "Quelles sont les étapes pour créer un organisme de formation en France ?", en: "What are the steps to create a training organization in France?" },
      a: { fr: "Créer un organisme de formation suppose de réaliser une première convention de formation, de déposer la déclaration d'activité auprès de la DREETS dans les trois mois, d'obtenir le numéro de déclaration d'activité, puis d'engager la démarche Qualiopi si l'accès aux fonds publics et mutualisés est visé. La certification Qualiopi est obligatoire depuis le 1er janvier 2022 pour accéder aux financements OPCO, CPF, France Travail et régions.", en: "Creating a training organization requires a first training agreement, filing the activity declaration with the DREETS within three months, obtaining the activity number, then starting the Qualiopi process if public funding is targeted. Qualiopi has been mandatory since January 1, 2022 to access OPCO, CPF and public funding." },
    },
    {
      q: { fr: "Combien de temps faut-il pour obtenir la certification Qualiopi ?", en: "How long does it take to obtain Qualiopi certification?" },
      a: { fr: "La démarche Qualiopi prend en moyenne trois à six mois entre l'engagement de la préparation et la délivrance de la certification, selon la maturité documentaire de l'organisme. Un audit blanc préalable permet d'identifier les non-conformités et de construire un plan d'action ciblé avant l'audit officiel.", en: "The Qualiopi process takes on average three to six months from preparation to certification, depending on the organization's documentary maturity. A mock audit helps identify non-conformities beforehand." },
    },
    {
      q: { fr: "Mentivis accompagne-t-il uniquement les nouvelles structures ou aussi les OF existants ?", en: "Does Mentivis support only new structures or also existing training organizations?" },
      a: { fr: "Mentivis intervient à toutes les étapes : création de A à Z pour les porteurs de projet, structuration et mise en conformité Qualiopi pour les OF existants, et développement commercial pour les organismes en phase de croissance.", en: "Mentivis intervenes at every stage: end-to-end creation for project holders, structuring and Qualiopi compliance for existing training organizations, and commercial development for scaling organizations." },
    },
    {
      q: { fr: "Quel est le coût d'un accompagnement Qualiopi avec Mentivis ?", en: "What is the cost of Qualiopi support with Mentivis?" },
      a: { fr: "Le modèle tarifaire de Mentivis intègre une part fixe de structuration et une part variable alignée sur les résultats : certification obtenue, apprenants recrutés. Le premier échange est sans engagement et permet de cadrer précisément le périmètre et le coût de la mission.", en: "Mentivis pricing includes a fixed structuring part and a variable part aligned with results: certification obtained, learners recruited. The first exchange is non-binding." },
    },
  ];
  return (
    <>
      ...
      <JsonLd
        data={{
          ...
          mainEntity: faq.map((item) => ({
            "@type": "Question",
            name: isFr ? item.q.fr : item.q.en,
            acceptedAnswer: {
              "@type": "Answer",
              text: isFr ? item.a.fr : item.a.en,
            },
          })),
        }}
      />
      ...
      <PageShell>
        <section style={{ padding: "120px 24px 60px", maxWidth: 780, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gap: 24 }}>
            {faq.map((item, i) => (
              <div key={i}>
                <h3 style={{ fontSize: 17, fontWeight: 600, margin: "0 0 8px", color: "var(--m-ink)" }}>{isFr ? item.q.fr : item.q.en}</h3>
                <p style={{ lineHeight: 1.7, color: "var(--m-ink-2)", margin: 0 }}>{isFr ? item.a.fr : item.a.en}</p>
              </div>
            ))}
          </div>
        </section>
