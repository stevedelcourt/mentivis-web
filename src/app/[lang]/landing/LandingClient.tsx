...
function ContactForm({ lang, c }: { lang: string; c: any }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const ok = await hsSubmit(
      ...
      {
        pageUri: typeof window !== "undefined" ? window.location.href : "https://mentivis.com/fr/landing/",
        pageName: "Landing",
      }
    );
...
const LANDING_HERO = {
  fr: {
    title: ["Vous gérez un OF.", "On peut vous aider.", "Dites-nous où vous en êtes."],
    lead: "Un échange d'une heure. Sans engagement. Sans présentation commerciale. Vous parlez de votre situation, on vous dit franchement si on peut faire quelque chose et comment.",
  },
  en: {
    title: ["You run a training organization.", "We can help.", "Tell us where you stand."],
    lead: "A one-hour conversation. No commitment. No sales deck. You talk about your situation, we tell you frankly whether and how we can help.",
  },
} as const;
...
export default function LandingClient() {
  ...
  const c = t.contact;
  const hero = LANDING_HERO[lang as "fr" | "en"] || LANDING_HERO.fr;
  ...
  return (
    <PageShell hidePreFooterCTA>
      <JsonLd data={{
        ...
        "@type": "ContactPage",
        name: hero.title.join(" "),
        description: hero.lead,
        url: `${SITE.baseUrl}/${lang}/landing/`,
      ...
      <section style={{ paddingTop: 120, paddingBottom: 60 }}>
        <div className="container">
          ...
          <Reveal delay={160}>
            <p className="t-lead" style={{ marginTop: 28, maxWidth: 580 }}>{hero.lead}</p>
          </Reveal>
        </div>
      </section>
